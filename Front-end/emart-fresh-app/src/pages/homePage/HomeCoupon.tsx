  ...
  useEffect(() => {
    if (isLogined) {
      sendAxiosRequest(
        ...
        setLoginToken,
        setCartCount,
        { page: currentPage, size: numberPerPage }
      )
        .then((res) => {
          ...
          const response: ExtendedCoupon[] = JSON.parse(
      ...
      axios
        .get(`${import.meta.env.VITE_BACK_PORT}/coupon/coupon-all`, {
          params: {
            page: currentPage,
            size: numberPerPage,
          },
          ...
          {couponData.map((coupon) => {
            return (
              <div
                key={coupon.couponId}
                className={`${
                  coupon.existing ? styles.unableCoupon : styles.coupon
                }`}
              >
                ...
                {coupon.admin ? (
                  ...
                  <button
                    className={`${
                      coupon.existing
                        ? styles.unableDownBtn
                        : styles.couponDownloadBtn
                    }`}
